...
@app.post("/dispatch")
async def dispatch_ride(request: Request):
    body = await request.json()
    request_id = request.headers.get("X-Request-ID", str(uuid.uuid4()))

    log_event("ride_dispatch_started", request_id, extra={"dispatch_payload": body})
    ...
    try:
        ...
        log_event(
            "dispatch_callback_sent_to_service_a",
            request_id,
            status="success",
        ...
        log_event(
            "dispatch_callback_to_service_a_failed",
            request_id,
            status="error",
            extra={"error": str(e)},
        )

    return JSONResponse(content=dispatch_result)
